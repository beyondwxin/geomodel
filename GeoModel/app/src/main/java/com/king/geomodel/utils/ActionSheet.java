...
public class ActionSheet extends Fragment implements OnClickListener {
    ...
    private void createItems() {
        ...
        if (titles != null) {
            for (int i = 0; i < titles.length; i++) {
                ...
                bt.setText(titles[i]);
                if ("删除".equals(titles[i]) || "举报".equals(titles[i])) {
                    bt.setTextColor(getResources().getColor(R.color.base_red));
                } else {
    ...
    public static class Builder {
        ...
        public Builder setOtherButtonTitles(int... resIds) {
            String[] strs = new String[resIds.length];
            for (int i = 0; i < resIds.length; i++) {
                int res = resIds[i];
                strs[i] = mContext.getString(res);
            }
            mOtherButtonTitles = strs;
            return this;
        }
