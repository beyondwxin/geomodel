...
public class ManageBucketSamples {
    ...
    public void deleteNotEmptyBucket() {
        ...
        PutObjectRequest putObjectRequest = new PutObjectRequest(bucketName, CommonValues.UPLOADOBJECT, uploadFilePath);
        try {
            ...
            public void onFailure(DeleteBucketRequest request, ClientException clientException, ServiceException serviceException) {
                ...
                if (serviceException != null) {
                    ...
                    if (serviceException.getStatusCode() == 409) {
                        // 删除文件
                        DeleteObjectRequest deleteObjectRequest = new DeleteObjectRequest(bucketName, CommonValues.UPLOADOBJECT);
                        try {
    ...
    public boolean deleteBucket(int count) {
        String objectKey = CommonValues.UPLOADOBJECT + "/" + "academy_" + count + ".jpg";
        DeleteObjectRequest deleteObjectRequest = new DeleteObjectRequest(CommonValues.ACADEMYBUCKET, objectKey);

        OSSAsyncTask deleteObjectTask = oss.asyncDeleteObject(deleteObjectRequest, new OSSCompletedCallback<DeleteObjectRequest, DeleteObjectResult>() {
            @Override
